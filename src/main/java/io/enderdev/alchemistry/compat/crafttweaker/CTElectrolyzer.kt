...
object CTElectrolyzer {
	...
	@JvmStatic
	fun addRecipe(
		input: ILiquidStack, electrolyte: IItemStack, electrolyteConsumptionChance: Int,
			...
			override fun apply() {
				...
				if(inputStack != null && electrolytestack != null && output1Stack != null && output2Stack != null) {
					val recipe = ElectrolyzerRecipe(
						inputStack, Ingredient.fromStacks(electrolytestack), electrolyteConsumptionChance,
						output1Stack, output2Stack, output3Stack, output3Chance ?: 0, output4Stack, output4Chance
							?: 0
					)
					ElectrolyzerRegister.Companion.INSTANCE.recipes.add(recipe)
		...
		Alchemistry.LATE_REMOVALS.add(object : IAction {
			...
			override fun apply() {
				...
				if(inputStack != null && electrolyteStack != null) {
					ElectrolyzerRegister.Companion.INSTANCE.recipes.removeIf { it.input.isFluidEqual(inputStack) && it.matchesElectrolyte(electrolyteStack) }
				}
			}
		})
	}
	...
	fun removeAllRecipes() {
		Alchemistry.LATE_REMOVALS.add(object : IAction {
			...
			override fun apply() = ElectrolyzerRegister.Companion.INSTANCE.recipes.clear()
		})
	}
}
